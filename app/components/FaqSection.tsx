...
export function FaqSection() {
  ...
  return (
    <section className="relative z-10 w-full bg-white py-12 sm:py-24 border-t border-gray-100 overflow-hidden font-sans">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Title */}
        <div className="mb-8 sm:mb-12">
          <h2 className="text-xl text-center sm:text-3xl lg:text-4xl font-normal text-primary tracking-tight">
            ...
            return (
              <div key={index} className="py-3.5 sm:py-5">
                <button
                  ...
                  className="w-full flex justify-between items-center text-left cursor-pointer select-none focus:outline-none group min-h-[44px]"
                  aria-expanded={isOpen}
                >
                  <span className="text-xs sm:text-base font-normal text-gray-900 tracking-tight pr-4 sm:pr-6 group-hover:text-black leading-snug">
                    {faq.question}
                  </span>

                  {/* Clean + / - indicator */}
                  <span className="text-lg sm:text-2xl font-light text-gray-800 shrink-0 select-none pl-2">
                    {isOpen ? '−' : '+'}
                  </span>
                </button>
                ...
                {/* Answer Accordion */}
                <div
                  className={`grid transition-all duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr] opacity-100 mt-2 sm:mt-3' : 'grid-rows-[0fr] opacity-0'
                    }`}
                >
