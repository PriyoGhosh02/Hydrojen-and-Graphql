...
interface ImageWithTextProps {
  data: {
    image_1?: { value: string };
    title_1?: { value: string };
    desc_1?: { value: string };
    image_2?: { value: string };
...
export function ImageWithText({ data }: ImageWithTextProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Create a single master timeline for pinned scroll animations
      ...
      // Animate Image 2 sliding up from bottom to top over the entire scroll
      tl.to('.anim-image-2', { yPercent: 0, ease: 'none' }, 0);

      // Fade out Content 1 during the first half of the scroll
      ...
      // Fade in Content 2 during the second half of the scroll
  ...
  return (
    <section
      ref={sectionRef}
      className="relative z-10 bg-white min-h-screen flex items-center overflow-hidden w-full max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-10"
    >
